...
server side:
	...
	*/
	...
	var Highlighting = $.inherit(Annotation, /** @lends TemporalTagging# */{
  	...
  	__constructor : function(options, tag_obj) {
		...
		name : 'highlight',
		type : 'annotation',
		options : {selector: '#highlight', vizOnTimeline: true, max: 20, sort: 'alpha', order: 'asc'},
		player : null,
		tag_obj : {},
		tag_list : [],
		...
		appendToDOM : function(id){
			var _this = this;
			$(vi2.dom).find('[type="highlight"]').each(function(i,val){ $(this).remove(); }); 
		...
		displayTagcloud : function(){  
			...
			var ln = function(_name, _freq, _highlight){ 
				return $('<li></li>').addClass('list-item').attr('id', _name.replace(' ', '--')).append($('<a></a>').attr('freq', _freq).addClass('id-'+_name.replace(' ', '--'))
					...
					.bind('click', {highlight:_highlight}, function(e){ 
		...
		showTimelineTag : function(e){ 
			var _this= this;
			if(e.highlight.occ.length === 1 && e.highlight.occ[0] >= 0){
				// jump to temporal position 
				this.player.currentTime(e.highlight.occ[0]);
			}else{
				...
				$.each(e.highlight.occ, function(){
					var progress = this / _this.player.duration();
					progress = ((progress) * $(_this.timelineSelector).width());
  	    	if (isNaN(progress)) { progress = 0; }
	 				$(_this.timelineSelector).append(f(progress, e.highlight.tagname, this));
 				});
			}
		},
