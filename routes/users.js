...
exports.openLoginPage = function (req, res) {
	res.render('user-login', { user: req.user, username: false, password: false, message: req.flash('error') });
};
...
exports.getOnlineStatus = function (req, res) {
	Users.find({ username: req.params.username }).select('status').exec(function (err, item) {
		if (err) {
			console.log(err);
			res.end('error');
		} else {
...
exports.setOnlineStatus = function (req, res) {
	Users.findById(req.params.id).select('status').exec(function (err, user) {
