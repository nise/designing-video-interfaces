...
exports.csvImport = function (req, res) {
  ...
  fs.readFile(__dirname + "/../data/users.csv", function read(err, data) {
    ...
    Users.remove({}, function (err) {
      console.log("Removed Users from DB");
    });
    csv()
      ...
      .to.array(function (data) {
        ...
        for (var i = 1; i < data.length; i++) {
          new Users({
            ...
            username:
              data[i][13] === ""
                ? data[i][0].replace(/\ /g, ".").toLowerCase() +
                  "." +
  ...
  /*
	identicon.generate(name, 10, function(err, buffer) {
		 if(err){ console.log('## ERROR while generating identicon')}
			fs.writeFileSync(__dirname+'/../public/vi-lab/img/user-icons/user-'+id+'.png', buffer);
	});
	
	identicon.generate(name, 40, function(err, buffer) {
		 if(err){ console.log('## ERROR while generating identicon')}
			fs.writeFileSync(__dirname+'/../public/vi-lab/img/user-icons/user-'+id+'_big.png', buffer);
	});*/
};
...
exports.handleLogout = function (req, res) {
  req.session.regenerate(function () {
    require("../server").socketio("user.disconnected", req.user);
    req.logout();
    res.redirect("/login");
  });
};
      ...
      findByUsername(username, function (err, user) {
        ...
        if (user[0] !== undefined) {
          ...
          if (user.password !== password) {
            ...
            console.log(11);
            //var io = require('socket.io')(require('../server').getServer());
            //io.sockets.on('connection', function (socket) {							console.log(8888888888); socket.broadcast.emit('user.logged.in', {user: user.id});});
...
exports.registerUser = function (req, res) {
  req
    .checkBody("email", "Bitte geben Sie eine gültige E-Mail-Adresse an.")
    .isEmail();
  req
    .checkBody("username", "Bitte geben Sie einen Benutzernamen ein.")
    .notEmpty();
  req
    .checkBody(
      "username",
      "Bitte geben Sie einen Benutzernamen an, der zwischen 3 und 15 Zeichen lang ist."
    )
    .len(3, 15);
  req
    .checkBody("password", "Das Passwort sollte 6 bis 20 Zeichen umfassen.")
    .len(6, 20);
  ...
  new Users({
    ...
    if (err) {
      ...
      res.render("user-login", {
        ...
        password: req.body.password,
        message:
          "Ihre Registrierung war erfolgreich. Sie können sich jetzt einloggen.",
      });
      //res.redirect('/login');
    }
...
exports.setOnlineStatus = function (req, res) {
  Users.findById(req.params.id)
    .select("status")
    .exec(function (err, user) {
      user.status.online = req.body.online_status;
      user.status.location = req.body.online_location;
      user.updated_at = Date.now();
      user.save(function (err, user, count) {
        res.end("updated user online status");
      });
    });
};
