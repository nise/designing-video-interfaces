    ...
    .exec(function (err, docs) {
      ...
      for (var i = 0; i < docs.length; i++) {
        count_patterns_per_portal +=
          String(docs[i].patterns).split(",").length + 1 + ",";
      ...
      var csv = "portal," + structs.pattern_names.toString(),
        csv2 = csv;
      for (i in m) {
    ...
    .exec(function (err, docs) {
      ...
      for (var i = 0; i < docs.length; i++) {
        if (selectedPortalsSubset.indexOf(docs[i].name) != -1) {
          for (var p = 0; p < docs[i].patterns.length; p++) {
            if (
              selectedPortalsSubset.indexOf(docs[i].name) != -1 &&
              docs[i].patterns[p] != ""
            ) {
              val = 1;
            } else {
            ...
            if (pattern_arr[docs[i].patterns[p]] !== undefined) {
              pattern_arr[docs[i].patterns[p]][
                Number(selectedPortalsSubset.indexOf(docs[i].name))
              ] = val;
            }
          }
        }
      ...
      for (var i in pattern_arr) {
        if (pattern_arr.hasOwnProperty(i)) {
          out +=
            selectedPortalsSubset[ii] + "," + pattern_arr[i].toString() + "\n";
          ii++;
        }
      }
      //console.log(out);
      write2file(
        "pattern-interrater_blanc.csv",
        out,
        "/home/abb/Documents/proj_001_doc/work/tables/"
      );
    });
};
...
exports.kappa = function (req, res) {
  fs.readFile(
    "/home/abb/Documents/proj_001_doc/work/tables/pattern-interrater_as.csv",
    function read(err, data) {
      if (err) {
        console.log(err);
      }
      csv()
        .from.string(data, { comment: "#" })
        .to.array(function (data) {
          ...
          paff.push(
            "$\\kappa$ & \\rotatebox{90}{" +
              portal_kappas.toString().replace(/,/g, "~} &\\rotatebox{90}{") +
              "}\\\\"
          );
          paff.push("\\hline");
...
var patternGroupStats = function (patterns) {
  var stat = {
    Grundfunktionen: 0,
    "Zugang zu zeitbasierter Information": 0,
    Kontribution: 0,
    Strukturierung: 0,
    Selbstorganisation: 0,
    Layout: 0,
  };
  var stat2 = {};
  ...
  Portals.find()
    ...
    .exec(function (err, docs) {
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        out_latex +=
          "\\item \\textbf{" +
          docs[i].name.toString().replace(/&/g, "\\&") +
          "}\\\\\nProvider: " +
          docs[i].provider.toString().replace(/&/g, "\\&") +
          "\\\\\nURL: \\href{" +
          docs[i].url +
          "}{" +
          docs[i].url.toString().replace(/\_/g, "\\_") +
          "} (abgerufen am 15.01.2016)\\\\\nGruppierung: " +
          docs[i].tags.toString().replace(/,/g, ", ") +
          "\\\\\nPatterns: " +
          docs[i].patterns.toString().replace(/,/g, ", ") +
          "\\\\\nFunktionalitätsrate: " +
          fn_rate +
          "\n";
      ...
      for (var i in structs.pattern_engagement_taxonomie) {
        if (structs.pattern_engagement_taxonomie.hasOwnProperty(i)) {
          columns.push({
            head: i,
            cl: structs.pattern_engagement_taxonomie[i]
              .toLowerCase()
              .replace(/\ /g, "-"),
            html: i,
          });
      ...
      console.log(out_patterns);
      res.render("analysis_portal-pattern-table", {
        data: out_patterns,
        columns: columns,
      });
...
exports.getInstancesOfPattern = function (group) {
  ...
  Portals.aggregate([
    ...
    for (k in data) {
      ...
      if (data.hasOwnProperty(k) && data[k]._id !== "") {
        ...
        if (structs.pattern_dim[data[k]._id] in absolute === false) {
          ...
          absolute[structs.pattern_dim[data[k]._id]].abs_frequency += Number(
            data[k].count
          );
        }

        all.push(Number(data[k].count));
        absolute[structs.pattern_dim[data[k]._id]].values.push(
          Number(data[k].count)
        );
        ...
        color =
          structs.micro_pattern_names.indexOf(data[k]._id) !== -1
            ? "light-gray"
            : "dark-gray";
        console.log(
          "" +
            (structs.dimensions.indexOf(structs.pattern_dim[data[k]._id]) + 1) +
            " & \\textsc{" +
            data[k]._id +
            "} & " +
            data[k].count +
    ...
    var out = "",
      ...
      sum_abs = 0;
    var groupp =
      structs.portal_groups[group] == undefined
        ? ""
        : "in der Gruppe " + structs.portal_groups[group].replace("Video ", "");
    out +=
      "{ } & \\multicolumn{5}{c}{Kategorien " + groupp + "} \\\\\\cline{2-6}\n";
    out +=
      "& {Anzahl} & {abs. Häufigkeit} & rel. Häufigkeit/\\% & \\textit{ M} & \\textit{ SD}\\\\\\hline\n";

    for (var a in absolute) {
      if (absolute.hasOwnProperty(a)) {
        absolute[a].rel_frequency = (
          (absolute[a].abs_frequency / sum_abs_frequency) *
          100
        ).toFixed(2); //console.log(a)
        ...
        out +=
          a +
          " & " +
          absolute[a].num_patterns +
          " & " +
          absolute[a].abs_frequency +
          " & " +
          absolute[a].rel_frequency +
          " & " +
          absolute[a].mean +
    ...
    out +=
      ...
      sum_abs_frequency +
      " & 100 & " +
      new gauss.Vector(all).mean().toFixed(2) +
      " & " +
      new gauss.Vector(all).stdev().toFixed(2) +
      "\\\\\n";
    out += "\\hline\\\\\n";
    ...
    write2file(
      "pattern-ming-" +
        String(structs.portal_groups[group]).toLowerCase().replace(/\ /g, "-") +
        ".tex",
      out,
      "/home/abb/Documents/proj_001_doc/work/tables/"
    );
    ...
    .exec(function (err, docs) {
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        for (var j = 0; j < tags.length; j++) {
          if (c.indexOf(tags[j]) === -1) c.push(tags[j]);
        }
      }
      // rearrange c
    ...
    .exec(function (err, docs) {
      ...
      for (a in c) {
        if (c.hasOwnProperty(a)) {
          ...
          json.nodes.push({
            name: c[a],
            group: String(structs.pattern_dim[c[a]]).length,
          });
        }
      }
      //console.log(json)
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        for (t in permuted_patterns) {
          if (
            permuted_patterns.hasOwnProperty(t) &&
            permuted_patterns[t].length === 2
          ) {
            patterns = permuted_patterns[t];
            if (
              c.indexOf(patterns[1]) !== -1 &&
              c.indexOf(patterns[0]) !== -1
            ) {
              // start with the greatest number
  ...
  Portals.find()
    ...
    .exec(function (err, portals) {
      ...
      for (var i = 0; i < portals.length; i++) {
        json.nodes.push({
          name: String(portals[i].name).replace("'", ""),
          group: 1,
        });
        portal_names += '"' + String(portals[i].name).replace("'", "") + '",';
        if (i == 0) {
          for (k = 0; k < portals.length; k++) {
            str += String(portals[k].name).replace("'", "") + ",";
          }
          str += "\n";
        }
        for (var j = 0; j < portals.length; j++) {
          if (j == 0) {
            str += String(portals[i].name).replace("'", "") + ",";
          }
          if (portals[i]._id !== portals[j]._id) {
            // prepare pattern vector
            vi = getPatternVector(
              portals[i].patterns,
              structs.micro_pattern_names
            ); // structs.macro_pattern_names
            vj = getPatternVector(
              portals[j].patterns,
              structs.micro_pattern_names
            );
            ...
            var union = portals[i].patterns.concat(
              portals[j].patterns.filter(function (item) {
    ...
    .exec(function (err, patterns) {
      ...
      for (var i = 0; i < patterns.length; i++) {
        nodes +=
          ...
          patterns[i].name +
          "," +
          structs.pattern_dim[patterns[i].name] +
          "\n";
        for (var j = 0; j < patterns[i].related_patterns.length; j++) {
          ...
          // debug
          if (index[rel.label] == null) {
            console.log(patterns[i].name + "___-" + rel.label + "\n\n");
          }
        }
      }
      console.log(nodes);
      console.log(edges);
      write2file(
        "pattern_relations_nodes,csv",
        nodes,
        "/home/abb/Documents/www2/video-patterns/data/"
      );
      write2file(
        "pattern_relations_edges,csv",
        edges,
        "/home/abb/Documents/www2/video-patterns/data/"
      );
    });
...
write2file = function (filename, dataset, new_path) {
  ...
  if (!filename || !dataset) {
    console.log("No data or file to write");
    return;
  }
