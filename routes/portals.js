exports.index = function (req, res) {
  res.render("portals", { title: "Express Portals Example", production: 1 });
};

//var utils    = require( '../utils' );
var mongoose = require("mongoose"),
  Portals = mongoose.model("Portals"),
  fs = require("node-fs"),
  csv = require("csv");
/*
Import Portals data from csv
**/
exports.csvImport = function (req, res) {
  return;
  // load data
  fs.readFile(
    __dirname + "/../data/videoportals.csv",
    function read(err, data) {
      if (err) {
        console.log(err);
      }
      // get Video dataset in order to extract toc data
      var tags = [];
      // destroy dataset first
      Portals.remove({}, function (err) {
        console.log("collection removed");
      });
      csv()
        .from.string(data, { comment: "#" })
        .to.array(function (data) {
          // define Portals for each line
          for (var i = 0; i < data.length; i++) {
            tags = [];

            if (data[i][10] != false) {
              tags.push("Video Learning Environment");
            }
            if (data[i][11] != false) {
              tags.push("Video Portal");
            }
            if (data[i][12] != false) {
              tags.push("Video Authoring Environment");
            }
            if (data[i][13] != false) {
              tags.push("Video Framework");
            }
            if (data[i][14] != false) {
              tags.push("Interactive Film");
            }
            if (data[i][15] != false) {
              tags.push("Online Environment");
            }
            if (data[i][16] != false) {
              tags.push("Desktop Environment");
            }
            if (data[i][17] != false) {
              tags.push("Research Prototype");
            }
            if (data[i][18] != false) {
              tags.push("Annotation Tool");
            }
            if (data[i][19] != false) {
              tags.push("Search Engine");
            }
            if (data[i][20] != false) {
              tags.push("Misc");
            }
            if (data[i][7] != false) {
              tags.push("Open Source");
            }
            new Portals({
              id: data[i][0],
              name: data[i][1],
              description: data[i][2],
              category: data[i][3],
              provider: data[i][4],
              url: data[i][5],
              accessible: data[i][6], // xxx update necessary
              availability: data[i][9], // xxx update necessary
              tags: tags,
              patterns: String(data[i][8]).split(", "),

              updated_at: Date.now(),
            }).save(function (err, todo, count) {
              //res.redirect( '/portals' );
            });
          }
        }); // end array()
      console.log("Imported videoportals.csv");
      console.log("......................");
    }
  ); // end fs
};

/*
MAINTANANCE
*/

exports.maintain = async function (req, res) {
  res.end(); // xxx
  var up = require("../data/2016_08_25_backup_portals.json");
  aop = function (name) {
    for (var i = 0; i < up.length; i++) {
      if (up[i].name === name) {
        return up[i].availability;
      }
    }
  };

  try {
    const p = await Portals.find().exec();

    for (var i = 0; i < p.length; i++) {
      p[i].usability = {};
      p[i].usability.open_source =
        p[i].tags.indexOf("Open Source") === -1 ? false : true;
      p[i].analysis = {};
      p[i].analysis.availability = aop(p[i].name);
      //console.log(p[i].usability.open_source)
      await p[i].save();
    }
  } catch (err) {
    console.error(err);
  }
};

/*
REST API CALL
**/
exports.getJSON = async function (req, res) {
  try {
    const portals = await Portals.find().sort("id").lean().exec();
    res.jsonp(portals);
  } catch (err) {
    console.error(err);
    res.status(500).jsonp({ error: "Error fetching portals" });
  }
};

/*
 * View a single portal
 **/
exports.view = async function (req, res) {
  try {
    const portal = await Portals.findOne({ _id: req.params.id }).exec();
    if (!portal) {
      return res.status(404).send("Portal not found");
    }
    res.render("portals-single", { items: portal, production: 1 });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portal");
  }
};

/*
 **/
exports.getJSONPortalsOfPattern = async function (req, res) {
  try {
    const docs = await Portals.find({
      patterns: String(req.params.id).replace(/-/g, " "),
    })
      .sort("name")
      .lean()
      .exec();
    res.type("application/json");
    res.jsonp(docs);
  } catch (err) {
    console.error(err);
    res.status(500).jsonp({ error: "Error fetching portals" });
  }
};

/***/
exports.getPortalsOfPattern = async function (req, res) {
  try {
    const docs = await Portals.find({
      patterns: String(req.params.id).replace(/-/g, " "),
    })
      .sort("id")
      .lean()
      .exec();
    res.render("portals", {
      title:
        docs.length +
        ' video learning environments containing the component or pattern <a href="/patterns/view/' +
        req.params.id +
        '" >' +
        req.params.id.replace(/-/, " ") +
        "</a>",
      items: docs,
      production: 1,
    });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portals");
  }
};

/***/
exports.getPortalsOfTag = async function (req, res) {
  try {
    const docs = await Portals.find({
      tags: String(req.params.id).replace(/-/g, " "),
    })
      .sort("id")
      .lean()
      .exec();
    res.render("portals", {
      title:
        docs.length +
        ' video learning environments categorizes as "' +
        req.params.id +
        '"',
      items: docs,
      production: 1,
    });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portals");
  }
};

exports.getJSONPatterns = function (req, res) {
  res.end("done");
}; // xxx

/*
 **/
exports.getJSONPortalNames = async function (req, res) {
  try {
    const portals = await Portals.find({})
      .select("name -_id")
      .sort("name")
      .exec();

    var arr = [];
    for (var p in portals) {
      if (portals.hasOwnProperty(p)) {
        arr.push(portals[p].name);
      }
    }
    res.jsonp(arr);
  } catch (err) {
    console.error(err);
    res.status(500).jsonp({ error: "Error fetching portal names" });
  }
};

//
exports.create = async function (req, res) {
  try {
    await new Portals({
      name: req.body.name,
      description: req.body.description,
      category: "-99", //req.body.category;
      provider: req.body.provider,
      url: req.body.url,
      accessible: req.body.accessible,
      open_source: false,
      availability: req.body.availability,
      tags: req.body.tags,
      patterns: String(req.body.patterns).split(","),
      updated_at: Date.now(),
    }).save();
    res.redirect("/portals");
  } catch (err) {
    console.error(err);
    res.status(500).send("Error creating portal");
  }
};

// query db for all todo items
exports.list = async function (req, res) {
  try {
    const docs = await Portals.find().sort("name").exec();
    res.render("portals", {
      title: "Video Learning Environments (" + docs.length + ")",
      items: docs,
      production: 1,
    });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portals");
  }
};

// new
exports.new_one = async function (req, res) {
  try {
    const todos = await Portals.find().sort("number").exec();
    res.render("portals-new", {
      title: "Express Portals Example",
      todos: todos,
      production: 1,
    });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portals");
  }
};

// remove todo item by its id
exports.destroy = async function (req, res) {
  try {
    const todo = await Portals.findById(req.params.id);
    if (!todo) {
      return res.status(404).send("Portal not found");
    }
    await todo.remove();
    res.redirect("/portals");
  } catch (err) {
    console.error(err);
    res.status(500).send("Error deleting portal");
  }
};

exports.edit = async function (req, res) {
  try {
    const item = await Portals.findOne({ _id: String(req.params.id) });
    if (!item) {
      return res.status(404).send("Portal not found");
    }
    res.render("portals-edit", {
      item: item,
      current: req.params.id,
      production: 1,
    });
  } catch (err) {
    console.error(err);
    res.status(500).send("Error fetching portal for editing");
  }
};

// redirect to index when finish
exports.update = async function (req, res) {
  try {
    const portal = await Portals.findById(req.params.id);
    if (!portal) {
      return res.status(404).send("Portal not found");
    }

    portal.name = req.body.name;
    portal.description = req.body.description;
    portal.provider = req.body.provider;
    portal.url = req.body.url;
    portal.accessible = req.body.accessible;
    portal.availability = req.body.availability;
    portal.tags = String(req.body.tags).split(",");
    portal.patterns = String(req.body.patterns).split(",");
    portal.updated_at = Date.now();

    await portal.save();
    res.redirect("/portals");
  } catch (err) {
    console.error(err);
    res.status(500).send("Error updating portal");
  }
};
