...
exports.addPattern = function (req, res) {
  Portals.find().exec(function (err, docs) {
    ...
    for (var i = 0; i < docs.length; i++) {
      ...
      portal.patterns = (
        String(docs[i].patterns).replace(/,Basic Controls/g, "") +
        ",Basic Controls"
      ).split(",");
      /*
    ...
    .exec(function (err, docs) {
      ...
      for (var i = 0; i < docs.length; i++) {
        ...
        if (String(docs[i].patterns).split(",").indexOf("") !== -1) {
          console.log(
            "Portal includes a blanc pattern / has empty pattern name: " +
              docs[i].name
          );
        }
      ...
      console.log(
        "Patterns that have not been mentioned: " + p_names.toString()
      );
      console.log("---------------------------");
  ...
  Portals.find()
    ...
    .exec(function (err, docs) {
      ...
      for (var c in portal_groups) {
        if (portal_groups.hasOwnProperty(c)) category[portal_groups[c]] = "";
      }
